...
#
#
# #18层和34层残差结构 与 50层级以上的残差层结构
# #for 18-layers and 34-layers
# class BasicBlock(nn.Module):
#     #主分支采用的卷积核个数是否有变化？
#     expansion = 1
#     #输入通道，输出通道，步长，下采样（表示虚线结构的shortcut
#     def __init__(self , in_channel , out_channel , stride = 1 , downsample=None):
#         super(BasicBlock, self).__init__()
#         self.conv1 = nn.Conv2d(in_channels=in_channel , out_channels=out_channel,
#                                kernel_size=3 , stride=stride, padding=1 , bias=False)
...
#
#         return out
# #for 50-layers , 101-layers , 152-layers
# class Bottleneck(nn.Module):
#     expansion = 4
#
#     def __init__(self , in_channel , out_channel , stride=1 , downsample=None):
...
# class ResNet(nn.Module):
#     #根据定义的层结构传入不同的块
#     #include_top在ResNet网络基础搭建更为复杂的网络
...
#
#         self.layer1 = self._make_layer(block , 64 , blocks_num[0])
#         self.layer1 = self._make_layer(block, 128, blocks_num[1] , stride=2)
